<template>
    ...
    >
        <template v-for="(item, index) in data">
            ...
            <!-- 无二级子节点 -->
            <el-menu-item v-if="!isHidden(item) && (!item.children || isHiddenChildren(item))" :index="item.path" :key="index">
                <i :class="item.icon"></i>
                <span slot="title">{{ item.name }}</span>
            </el-menu-item>

            <!-- 有二级子节点 -->
            <el-submenu v-if="!isHidden(item) && (item.children && !isHiddenChildren(item))" :index="item.name" :key="index">
                <template slot="title"><i :class="item.icon"></i><span>{{ item.name }}</span></template>
                <template v-for="(item1, index1) in item.children">
                    <!-- 无三级菜单 -->
                    <el-menu-item v-if="!isHidden(item) && (!item.children || isHiddenChildren(item))" :index="`${item.path}/${item1.path}`" :key="index1"><i :class="item1.icon"></i>{{ item1.name }}</el-menu-item>
